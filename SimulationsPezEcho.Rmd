...
nicheCplot<-ggplot(phy_comp,aes(x=Phylo.Relatedness,y=P_A)) + geom_smooth(aes(group=I),alpha=.1,se=F,method="glm",family="binomial",formula=(y~(poly(x,2)))) + labs(y="Probability of Presence",x="Phylogenetic Distance to Nearest Species in the Assemblage") + theme_bw() + coord_cartesian(ylim = ylm)
...
nicheWCplot<-sim(trx,size=10,breadth=100,signalFlag=F,repulse=T,replicates=50,multiplot=T)
...
```{r,fig.height=4,fig.width=4}
nicheCplot<-sim(trx,size=10,breadth=100,signalFlag=T,repulse=T,replicates=50,multiplot=T,ylm=c(0,1)) + ggtitle("Strong Phylogenetic Signal and Repulsion in Occurance Trait")
```

#Simulation 3: No Phylogenetic Signal Followed by No Repulsion in Occurrence Trait

```{r,fig.height=4,fig.width=4}
nicheWCNRplot<-sim(trx,size=10,breadth=100,signalFlag=F,repulse=F,replicates=50,multiplot=T)
...
nicheCNRplot<-sim(trx,size=10,breadth=100,signalFlag=T,repulse=F,replicates=50,multiplot=T)
...
mod1<-sim(trx,size=10,breadth=100,signalFlag=F,repulse=T,replicates=50,multiplot=F,plotR=F)

mod2<-sim(trx,size=10,breadth=100,signalFlag=T,repulse=T,replicates=50,plotR=F,ylm=c(0,1)) 

mod3<-sim(trx,size=10,breadth=100,signalFlag=F,repulse=F,replicates=50,plotR=F)

mod4<-sim(trx,size=10,breadth=100,signalFlag=T,repulse=F,replicates=50,plotR=F)
...
**Looks like just the strong signal and the strong repulsion is more sensitive to tip size at 32 tips, let's explore this in a bit more detail. Let's increase the level of repulsion.**

```{r}
trx<-compute.brlen(stree(n=32,type="balanced"))
plot(trx)

nicheCplot<-sim(trx,size=20,breadth=100,signalFlag=T,repulse=T,replicates=5,multiplot=T,ylm=c(0,.5),repulseVal=100,signal=100) + ggtitle("Stronger Phylogenetic Signal and Stronger Repulsion in Occurance Trait")
nicheCplot
```
