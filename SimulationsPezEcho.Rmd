...
To compare the form of the function between distance to most closely related species and the probability of co-occurrence given known phylogenetic mechanisms shaping species presence. I will consider three models, assemblages with weak phylogenetic signal, with strong phylogenetic signal, and with phylogenetic signal and the addition of repulsion. 
...
opts_chunk$set(cache=TRUE, cache.path = 'SimulationsPez_cache/', fig.path='figure/',fig.width=11,fig.height=7)
...
sim.func<-function(){scape(trx, scape.size=7, g.center=1.2, g.range=1.2, g.repulse=1, wd.all=10, signal.center=TRUE, signal.range=TRUE, same.range=TRUE, repulse=FALSE,center.scale = 1.1, range.scale = 1.1, repulse.scale = 1, site.stoch.scale = 0, sd.center=1, sd.range=1,rho=NULL, th=th)}
...
```{r,fig.height=6}
sim.func<-function(){scape(trx, scape.size=7, g.center=.1, g.range=.1, g.repulse=10, wd.all=15, signal.center=TRUE, signal.range=TRUE, same.range=TRUE, repulse=TRUE,center.scale = 1, range.scale = 1, repulse.scale = 1, site.stoch.scale = 0, sd.center=1, sd.range=1,rho=NULL, th=th)}
...
nicheWCmpd<-ggplot(m.dat[m.dat$variable=='mpd.obs.z',],aes(x=value)) + geom_histogram() + geom_vline(xintercept=c(.00),col='red') + ggtitle("Weak Phylogenetic Signal and Repulsion") + xlim(-5,5)
nicheWCmpd
```
...
```{r,fig.height=6}
sim.func<-function(){scape(trx, scape.size=7, g.center=.7, g.range=.7, g.repulse=10, wd.all=15, signal.center=TRUE, signal.range=TRUE, same.range=TRUE, repulse=TRUE,center.scale = 1, range.scale = 1, repulse.scale = 1, site.stoch.scale = 0, sd.center=1, sd.range=1,rho=NULL, th=th)}
...
m.dat<-melt(mpdNicheC)
nicheCmpd<-ggplot(m.dat[m.dat$variable=='mpd.obs.z',],aes(x=value)) + geom_histogram() + geom_vline(xintercept=c(.00),col='red') + ggtitle("Strong Phylogenetic Signal and Repulsion") + xlim(-5,5)
nicheCmpd
```
...
Its hard to imagine a plausible scenerio where a trait would evolve with very strong phylogenetic signal, followed by very strong repulsion among closely related species. What seems more likely is that some trait has weak or moderate signal and due to elevate niche overlap, species compete for finite resoures. I will therefore restrict the parameter space to g.center(0,.5)
