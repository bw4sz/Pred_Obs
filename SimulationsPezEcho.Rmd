...
opts_chunk$set(cache=TRUE, cache.path = 'SimulationsPez_cache/', fig.path='figure/',fig.width=11,fig.height=6,fig.align='center')
...
sim.func<-function(){
  ...
  scape(trx, scape.size=size, g.center=signal, g.range=1, g.repulse=repulseVal, wd.all=breadth, signal.center=signalFlag, signal.range=F, same.range=F, repulse=repulse,center.scale = 1, range.scale = 1, repulse.scale = 1, site.stoch.scale = 0, sd.center=1, sd.range=1,rho=NULL, th=th)}
...
nicheCplot<-ggplot(phy_comp,aes(x=Phylo.Relatedness,y=P_A)) + geom_smooth(method="glm",family="binomial",formula=(y~(poly(x,2)))) + labs(y="Probability of Presence",x="Phylogenetic Distance to Nearest Species in the Assemblage") + theme_bw() + coord_cartesian(ylim = ylm)
...
nicheWCplot<-sim(trx,size=10,breadth=40,signalFlag=F,repulse=T,replicates=20,multiplot=T)
...
```{r,fig.height=4,fig.width=4}
nicheCplot<-sim(trx,size=10,breadth=40,signalFlag=T,repulse=T,replicates=20,multiplot=T,ylm=c(0,1)) + ggtitle("Strong Phylogenetic Signal and Repulsion in Occurance Trait")
```

#Simulation 3: No Phylogenetic Signal Followed by No Repulsion in Occurrence Trait

```{r,fig.height=4,fig.width=4}
nicheWCNRplot<-sim(trx,size=10,breadth=40,signalFlag=F,repulse=F,replicates=20,multiplot=T)
...
nicheCNRplot<-sim(trx,size=10,breadth=40,signalFlag=T,repulse=F,replicates=20,multiplot=T)
...
```{r,echo=FALSE,eval=F}
mod1<-sim(trx,size=10,breadth=40,signalFlag=F,repulse=T,replicates=20,multiplot=F,plotR=F)

mod2<-sim(trx,size=10,breadth=40,signalFlag=T,repulse=T,replicates=20,plotR=F,ylm=c(0,1)) 
mod3<-sim(trx,size=10,breadth=40,signalFlag=F,repulse=F,replicates=20,plotR=F)

mod4<-sim(trx,size=10,breadth=40,signalFlag=T,repulse=F,replicates=20,plotR=F)
...
th=.6
sim.func<-function(repulse,signal,scale){scape(trx, scape.size=20, g.center=signal, g.range=signal, g.repulse=repulse, wd.all=40, signal.center=TRUE, signal.range=FALSE, same.range=FALSE, repulse=TRUE,center.scale = 1, range.scale = 1, repulse.scale = 1, site.stoch.scale = 0, sd.center=1, sd.range=1,rho=NULL, th=th)} 
...
The grid size needs to increase to accomodate more species, so we are not artifically packing them into a tight space. 
...
```{r,fig.height=6}
trees(sp=8,type="balanced",size=10,breadth=10,replicates=50,ylm=c(0,1)) 
```

###Ladder

```{r,fig.height=6}
trees(sp=8,type="left",size=10,breadth=40,replicates=50,ylm=c(0,1)) 
```
...
```{r,fig.height=6}
trees(sp=16,type="balanced",size=20,breadth=50,replicates=50,ylm=c(0,.6)) 
```

###Ladder

```{r,fig.height=6}
trees(sp=16,type="left",size=20,breadth=50,replicates=50,ylm=c(0,.6)) 
```
...
```{r,fig.height=5}
trees(sp=32,type="balanced",size=20,breadth=200,replicates=50,ylm=c(0,.5)) 
```

###Ladder

```{r,fig.height=6}
trees(sp=32,type="left",size=20,breadth=200,replicates=50,ylm=c(0,.7)) 
```

Looks like just the strong signal and the strong repulsion is more sensitive to tip size at 32 tips, let's explore this in a bit more detail.

```{r}
trx<-compute.brlen(stree(n=32,type="balanced"))
plot(trx)

nicheCplot<-sim(trx,size=20,breadth=500,signalFlag=T,repulse=T,replicates=5,multiplot=T,ylm=c(0,1),repulseVal=100,signal=100) + ggtitle("Stronger Phylogenetic Signal and Stronger Repulsion in Occurance Trait")
nicheCplot
```

As species # increase, the level of repulsion and signal needs to increase to get the predicted pattern.

```{r}
trx<-compute.brlen(stree(n=32,type="left"))
plot(trx)
...
Once we increase the signal and repulsion, we recover the polynomial pattern we see with smaller trees.

#Conclusion
The polynomial pattern seems robust to tree size, although the level of repulsion needs to be increased as there are more species in the tree. This is due to more time since most recent common ancestory. The pattern is not very robust to ladder trees, this is because there are very few closely related species in a ladder tree. We would expect this pattern to be more possible in tippy trees that have many late branching taxa and a more even distribution of tip distances.
