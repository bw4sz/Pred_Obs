...
```{r,fig.height=4,fig.width=4,cache=FALSE}
l<-co_occur(melt(as.matrix(siteXspp)))
  

ggplot(data=l,aes(x=Phylo.Relatedness,y=P_A)) + geom_smooth(family="binomial",method="glm",formula=y~poly(x,2)) + theme_bw() + ggtitle("Observed Relationship")
...
ggplot(dat,aes(x=value)) + geom_histogram() + facet_wrap(~variable,scales="free") + theme_bw() + ggtitle("Null Distribution of GLM Covariates") + geom_vline(data=observed,aes(xintercept=value),col='red',linetype='dashed') + geom_vline(data=quants,aes(xintercept=lower),col='black',size=.7)+ geom_vline(data=quants,aes(xintercept=upper),col='black',size=.7)

observed
```

The red dashed line is the observed glm estimate for the intercept, x and x^2. The thin black line is the central 95th quantile. From these simulations I infer that the the probability of getting our result by the intertwined structure of the model is very low. This is compounded that to get our result we need to get both the x and x^2 terms, they are not independent, so the joint probability of getting our result is even more remote than just $\alpha=0.05$ would suggest.
...
ynew<-melt(ynew,id.var=c("s","y"))

conf<-group_by(ynew,s) %>% summarise(mean=mean(y),upper=quantile(y,0.975),lower=quantile(y,0.025))


#Observed
yobs<-data.frame(x=l$Phylo.Relatedness,y=trajF(observed[1,2],observed[2,2],observed[3,2],l$Phylo.Relatedness))

ggplot(data=conf,aes(x=s,y=mean)) + geom_ribbon(aes(ymin=lower,ymax=upper),fill='gray80') + theme_bw() + scale_color_discrete(guide='none') + labs(x="Phylogenetic distance to the closest related species",y="Probability of Presence") + geom_line(data=yobs,aes(x=x,y=y),size=1.5,linetype="dashed",col="red") + ggtitle("Observed versus null fit") + geom_line(aes(y=mean))
