...
site.space<-rep(c(20,40,60,80,100),2)
```
...
Define function compute co-occurrence in each row

#Sample trajectories
```{r}
traj<-group_by(mdat,Sites,Species,variable) %>% summarize(Lower=quantile(Estimate,0.025),Higher=quantile(Estimate,0.975),mean=mean(Estimate)) 

names(traj)[3]<-"parameter"
mtraj<-melt(traj,measure.vars=c("Lower","Higher","mean"))
mtraj<-dcast(mtraj,...~parameter)

#Write a function that computes a random trajectory
ggplot(mtraj,aes(x=))
head(dcast(traj,...~variable))
