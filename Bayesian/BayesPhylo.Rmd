...
ggplot(spec,aes(x=estimate,fill=Species)) + geom_histogram()
...
```{r}
comp<-list(SlopeIntercept=out_slopeintercept,Intercept=out_single,Constrained=out_slope)

compm<-melt(comp)
compm<-dcast(compm,...~variable)

ggplot(compm,aes(x=L1,y=mean,ymax=upper,ymin=lower)) + geom_pointrange() + facet_wrap(~parameter,scales="free")
```
