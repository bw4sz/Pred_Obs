...
#MCMC options
ni <- 40000  # number of draws from the posterior
nt <- 1    #thinning rate
nb <- 0  # number to discard for burn-in
nc <- 2  # number of chains
...
parsSingleS<-parsSingleS[!parsSingleS$Draw %in% 1:35000,]
...
di<-data.frame(parameter=levels(specSingleS$parameter),Species=levels(dat$Species))

specSingleS<-merge(specSingleS,di,by="parameter")
...
#MCMC options
ni <- 30000  # number of draws from the 
nt <- 5    #thinning rate
nb <- 0  # number to discard for burn-in
nc <- 2  # number of chains
...
parsUS<-parsUS[!parsUS$Draw %in% 1:5000,]
...
gdf<-group_by(specSdf,Species) %>% sample_n(1000) %>% filter(str_detect(parameter,"beta")) 
...
specCast<-lapply(specS,function(y){
  dpars<-dcast(y,...~parameter,value.var="estimate")
  samp<-dpars[sample(1:nrow(dpars),1000),]  
  colnames(samp)<-c("Draw","Chain","ind","Species","alpha","beta","gamma")
