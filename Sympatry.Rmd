...
makePlot<-function(sampl){
  ...
  op <- par(mar=rep(.01, 4))
  plot(elev.ca,axes=F,legend=F,box=F,ext=extent(rbind.SpatialPoints(orig,dest))*2)
  par(op)
  ...
  svg(filnam)
  op <- par(mar=rep(.01, 4))
  plot(elev.ca,axes=F,legend=F,box=F,ext=extent(rbind.SpatialPoints(orig,dest))*1.5)
  par(op)
