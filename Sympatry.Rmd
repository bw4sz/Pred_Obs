...
#label pairs
ggplot(meanC,aes(x=Time,y=cp)) + geom_point(size=4,alpha=.8,shape=1) + scale_shape_discrete(solid=F)+ geom_smooth(method="lm",aes(group=1)) + theme_bw() + xlab("Time since divergence between sister taxa (myr)") + scale_color_discrete(guide="none") + ylab("Average cost distance between localities") + geom_smooth(data=meanC[meanC$Time<10,],method="lm",aes(group=1),linetype="dashed",col="black") 
