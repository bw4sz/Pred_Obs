...
sim.func<-function(){scape(trx, scape.size=10, g.center=1, g.range=1, g.repulse=1, wd.all=25, signal.center=FALSE, signal.range=FALSE, same.range=FALSE, repulse=FALSE,center.scale = 1, range.scale = 1, repulse.scale = 1, site.stoch.scale = 0, sd.center=1, sd.range=1,rho=NULL, th=th)}
...
sim.func<-function(){scape(trx, scape.size=10, g.center=100, g.range=100, g.repulse=1, wd.all=25, signal.center=TRUE, signal.range=TRUE, same.range=TRUE, repulse=FALSE,center.scale = 1, range.scale = 1, repulse.scale = 1, site.stoch.scale = 0, sd.center=1, sd.range=1,rho=NULL, th=th)}
...
```{r,fig.height=10}
sim.func<-function(){scape(trx, scape.size=10, g.center=100, g.range=1, g.repulse=10, wd.all=25, signal.center=TRUE, signal.range=FALSE, same.range=TRUE, repulse=TRUE,center.scale = 1, range.scale = 1, repulse.scale = 1, site.stoch.scale = 0, sd.center=1, sd.range=1,rho=NULL, th=th)}
...
**combine data and save file**
```{r saving}

phy_lottery$Model<-"Lottery"
phy_nichec$Model<-"Niche Conservatism"
phy_niche_comp$Model<-"Niche Conservatism and Competition"

simall<-rbind_all(list(phy_lottery,phy_niche_comp,phy_nichec))

simall$Assemblage<-"Simulated"
write.csv(simall,"C:/Users/Ben/Documents/Pred_Obs/Bayesian/simdats.csv")
```
