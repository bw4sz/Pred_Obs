...
names(sdat)<-c("Env","Observed","Env + Dispersal","NoSignal.StrongRepulsion","StrongSignal.StrongRepulsion","NoSignal.NoRepulse","StrongSignal.NoRepulse")
...
ggplot(data=specdf[!specdf$Assemblage %in% c("Env","Env + Dispersal","Observed"),],aes(x=Relatedness,y=mean))+ geom_ribbon(alpha=.5,aes(ymin=lower,ymax=upper)) + geom_line() + theme_bw()+ labs(x="Phylogenetic distance to closest related species in an assemblage",y="Probability of Occurrence") + scale_fill_manual(guide='none') + facet_wrap(~AssemblageL,scales="free") 
