...
names(sdat)<-c("Env","Observed","Env + Dispersal","Strong","Weak")
...
dummy<-data.frame(Relatedness=.9,mean=c(.4,.25),AssemblageL="Lottery",ymin=c(.5,.2),ymax=c(.4,.2))

ggplot(data=specdf[!specdf$Assemblage %in% c("Env","Env + Dispersal","Observed"),],aes(x=Relatedness,y=mean))+ geom_ribbon(alpha=.5,aes(ymin=lower,ymax=upper)) + geom_line() + theme_bw()+ labs(x="Phylogenetic distance to closest related species in an assemblage",y="Probability of Occurrence") + scale_fill_manual(guide='none') + facet_wrap(~AssemblageL,scales="free")  + geom_blank(data=dummy,aes(x=Relatedness,y=mean))
