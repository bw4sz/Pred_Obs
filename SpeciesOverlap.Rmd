...
#split simulations inside
sdats<-lapply(sdat,function(x){
  split(x,x$Iteration)
})

names(sdats)<-c("Env","Observed","Env + Dispersal","No Signal & Strong Repulsion","Strong Signal & Strong Repulsion","No Signal & No Repulsion","Strong Signal & No Repulsion")

# remove inf values
sdats<-lapply(sdats,function(x){
  lapply(x,function(y){
...
p<-ggplot(x,aes(x=Phylo.Relatedness,y=P_A,col=I)) + geom_smooth(method="glm",family="binomial",formula=y~poly(x,2),alpha=) + theme_bw() + labs("x=Distance to closest related species in an assemblage",y="Probability of presence") + ggtitle("") + scale_col_discrete(show_guide="none")
return(p)
})
...
```{r run jags,results='hide',eval=F}

#There is no reason everyone needs the same number of runs
#Draws for each model
runsl<-rep(200000,400000,200000,200000,200000,200000,200000)
...
#for(x in 1:length(sdat)){
  out<-Bayes(presence=sdat[[x]]$P_A,distance=sdat[[x]]$Phylo.Relatedness,species=sdat[[x]]$Species,runs = runsl[x],burn=((runsl[x]/25)-2000),hier=modhier[x])
  return(out)
  #parall[[x]]<-out
}
stopCluster(cl)
...
ggplot(data=specdf[!specdf$Assemblage %in% c("Env","Env + Dispersal","Observed"),],aes(x=Relatedness,y=mean))+ geom_ribbon(alpha=.5,aes(ymin=lower,ymax=upper)) + geom_line() + theme_bw()+ labs(x="Phylogenetic distance to closest related species",y="Probability of Occurrence") + scale_fill_manual(guide='none') + facet_wrap(~AssemblageL,scales="free") 
