...
runsl<-rep(1000,1000,1000,1000,1000,1000,1000)

#Which model we want to draw , heir=T is for full model
modhier<-c(rep(T,3),rep(F,4))
...
#overlay together
figure4<-ggplot(data=specdf[specdf$Assemblage %in% c("Env","Env + Dispersal","Observed"),],aes(x=Relatedness,y=mean)) + geom_ribbon(alpha=.5,aes(ymin=lower,ymax=upper,fill=Assemblage)) + geom_line(aes(fill=Assemblage)) + theme_bw()+ labs(x="Phylogenetic distance to closest related species",y="Probability of Occurrence") + scale_fill_manual(values=c("grey80","grey60","black"),labels=c("Environment","Environment + Dispersal","Observed"))
figure4
...
dummy<-data.frame(Relatedness=.9,mean=c(.4,.25),AssemblageL="No Signal & No Repulsion",ymin=c(.05,.2),ymax=c(.4,.2))

figure3<-ggplot(data=specdf[!specdf$Assemblage %in% c("Env","Env + Dispersal","Observed"),],aes(x=Relatedness,y=mean))+ geom_ribbon(alpha=.5,aes(ymin=lower,ymax=upper)) + geom_line() + theme_bw()+ labs(x="Phylogenetic distance to closest related species",y="Probability of Occurrence") + scale_fill_manual(guide='none') + facet_wrap(~AssemblageL) + geom_blank(data=dummy,aes(ymin=ymin,ymax=ymax))
figure3
...
#match index with species names, this needs to be confirmed, i'm unsure about the index ordering, could have been sorted.
...
gdf<-group_by(specSdf,Species) %>% filter(str_detect(parameter,"gamma")) %>%summarize(Lower=quantile(estimate,0.025),Upper=quantile(estimate,0.975),mean=mean(estimate))

ord<-gdf%>% arrange(mean)
...
ggplot(gdf,aes(x=Species,col=Clade)) + geom_linerange(aes(ymin=Lower,ymax=Upper)) + geom_point(aes(y=mean)) + theme_bw() + theme(axis.text.x = element_text(angle = 65, hjust = 1)) + ylab("Polynomial Coefficient")
ggsave("PolynomialEstimates.svg",dpi=300,height=5,width=11)
...
specCast<-lapply(specS,function(y){
  ...
  R<-PA_m2[PA_m2$Species %in% unique(samp$Species) & PA_m2$P_R %in% "PA_Binary",]$Phylo.Relatedness
