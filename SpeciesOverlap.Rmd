...
system.time(all_models<-foreach(x=1:nlayers(modlist),.export=c("sp.lists","siteXspp.raster","pred_realized"),.errorhandling="pass",.packages=c("raster","maptools","reshape","dismo","picante","ggplot2","rasterVis","lattice")) %do%{
...
if(plots){print("Species Table_Nodispersal")}
...
system.time(all_modelsDD<-foreach(x=1:nlayers(modlist),.errorhandling="pass") %do%{ 
...
if(plots){print("Species Table including dispersal")}
...
#rename the melted column
colnames(all.species.data)[9]<-"Hyp"

#Set working directory to output folder
dir.create(paste(fold,"Results",sep="/"))
setwd(paste(fold,"Results",sep="/"))

if(plots){kable(head(all.species.data))}
...
#contigency table
contin<-table(PA_mult2$P_Apred,PA_mult2$PA_Binary,PA_mult2$Hyp,deparse.level=2)
...
#Name the columns
colnames(PA_m2)[c(9,10)]<-c("P_R","P_A")
...
Sensitivity analysis shows that suitabilty threshold of 80% is a reasonable value, compute predicted and realized distributions.

```{r}
#specified as 1-threshold/100.
...
colnames(simall)<-c("Locality","Species","P_A","Phylo.Relatedness","Hyp","Iteration","P_R")
```
...
parall<-lapply(sdat,FUN=function(x){
  ...
  test<-Bayes(presence = x$P_A,distance = x$Phylo.Relatedness,species=x$Species,runs = 10000,burn=8000,plots = TRUE,folder = fold)
})
```
