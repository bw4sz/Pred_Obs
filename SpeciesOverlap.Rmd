...
```{r Niche Models,eval=F}
SDM_SP(cell_size,inLocalities,envfolder,savefolder,env=c('bio_1','bio_4','bio_12',"bio_15"))
```
...
records_site<-names(which(apply(siteXspp.raster,1,sum,na.rm=TRUE) > 0))
...
CostPathMatrix<-log(dcast(m.costlist,X1~X2,value.var="value")[,-1])
...
Sensitivity analysis shows that suitability threshold of 95% is a reasonable value, compute predicted and realized distributions. The function is specified as 1-threshold/100.
...
**Black points = Point Localities used to create the niche models**
**Red points = True presence assemblage lists**
**Empty Blue Triangles = predicted assemblage list based on environmental suitabilitys**
**Filled Blue Triangles = predicted assemblage list based on environmental suitability, but outside of dispersal distance**
...
names(sdat)<-c("Env","Observed","Env + Dispersal","No Conservatism & Strong Repulsion","Strong Conservatism & Strong Repulsion","No Conservatism & No Repulsion","Strong Conservatism & No Repulsion")
...
p<-ggplot(x,aes(x=Phylo.Relatedness,y=P_A)) + geom_smooth(method="glm",method.args=list(family="binomial"),formula=y~poly(x,2),alpha=min(c(1,(1/max(x$Iteration)*10)))) + theme_bw() + labs("x=Distance to closest related species in an assemblage",y="Probability of presence") + ggtitle("") + scale_alpha(guide="none")
return(p)
})
...
This corresponding to the winbugs models:
```{r}
...
runsl<-rep(50000,50000,50000,5000,5000,5000,5000)
...
parall<-foreach(x=1:length(sdat),.errorhandling="pass") %dopar% {
  y<-sdat[[x]]
  out<-Bayes(presence=y$P_A,distance=y$Phylo.Relatedness,species=y$Species,Iteration=y$Iteration,runs =runsl[x],burn=((runsl[x]/25)-2000),hier=modhier[x])
  return(out)
}
stopCluster(cl)
...
In review, we were asked to provide a measure of Psuedo R^2. In general these measures can be tricky with hierarchical models because the variance is explictely modeled among groups and is therefore non-independent by design. However, as a measure of relative improvement from a null model a simple psudeo R^2 of 1- (residual deviance/null deviance) will be useful. For each of the models we computed a null deviance by fitting an intercept only model. We maintained the hierarchical structure to make the models comparable.
...
```{r secondmodel,echo=TRUE,eval=TRUE}

Bayes <-function(presence,species,Iteration,runs,burn,hier) {
  #needs libraries specified to run in parallel
  library(ggplot2)
  library(R2jags)
  library(reshape2)
  ...
  source(paste(gitpath,"Bayesian/NullDeviance.R",sep = "/"))
  ...
  function() {
  list(
  alpha = rep(1,Dat$s),intercept = 1,tauIntercept = 0
  )
  }
  
  #Parameters to track
  ParsStage <- c("intercept","alpha")
...
#Draws for each model
runsl<-rep(2000,2000,2000,2000,2000,2000,2000)
...
parnull<-foreach(x=1:length(sdat),.errorhandling="pass") %dopar% {
  y<-sdat[[x]]
  out<-Bayes(presence=y$P_A,species=y$Species,Iteration=y$Iteration,runs =runsl[x],burn=((runsl[x]/3)-40),hier=modhier[x])
  return(out)
}
stopCluster(cl)
```
...
```{r}
ps<-sapply(1:length(parall),function(x){
  resid<-parall[[x]]$BUGSoutput$sims.list$deviance
  null<-parnull[[x]]$BUGSoutput$sims.list$deviance
  p<-1-(mean(resid)/mean(null))
  return(p)
})
...
levels(specdf$AssemblageL)<-c("Environment","Environment + Dispersal","No Conservatism & No Repulsion","No Conservatism & Strong Repulsion","Observed","Strong Conservatism & No Repulsion","Strong Conservatism & Strong Repulsion")
...
# grab target species       
spkeep<-c("Eriocnemis.vestita","Heliangelus.exortis","Coeligena.lutetiae","Thalurania.furcata","Adelomyia.melanogenys","Amazilia.tzacatl","Metallura.tyrianthina","Coeligena.torquata")
keep<-data.frame(Type=c(rep("Increasing",4),rep("Declining",4)),Species=spkeep)
ex<-droplevels(ex[ex$L1 %in% keep$Species,])
```
...
ex$L1<-gsub(ex$L1,pattern="\\.",replacement=" ")
pts$L1<-gsub(pts$L1,pattern="\\.",replacement=" ")

#relevel
ex$L1<-factor(ex$L1,levels=gsub(keep$Species,pattern="\\.",replacement=" "))

p<-ggplot(ex,aes(x=x,y=mean)) + geom_line(col="black") + geom_line(aes(y=upper),col="gray",linetype="dashed") + geom_line(aes(y=lower),col="gray",linetype="dashed")+ theme_bw() + labs(x="",y="") + facet_wrap(~L1,nrow=2,ncol=4)
...
p
ggsave("SpeciesExample.jpg",dpi=300,height=4,width=10)
ggsave("SpeciesExample.svg",dpi=300,height=4,width=10)
```
