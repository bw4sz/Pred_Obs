...
Bayes<-function(presence,distance,species,runs,burn){
...
runsl<-c(150000,70000,200000,70000,150000,150000)
runsl-2000

cl<-snow::makeCluster(7,"SOCK")
doSNOW::registerDoSNOW(cl)
parall<-foreach(x=1:length(sdat),.errorhandling="stop") %dopar% {
#create folder name
  fold<-paste(unique(sdat[[x]]$P_R),"_",unique(sdat[[x]]$Hyp),".txt",sep="")
  print(fold)
  sink(fold)
  test<-Bayes(presence = sdat[[x]]$P_A,distance = sdat[[x]]$Phylo.Relatedness,species=sdat[[x]]$Species,runs = runsl[x],burn=runsl[x]-2000)
  sink()
  return(test)
}
stopCluster(cl)
...
ggplot(parmelts,aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + facet_grid(L1~parameter,scales="free") + theme_bw() + labs(col="Chain")
```
...
finaltab<-group_by(parmelts,L1,Assemblage,parameter) %>% summarize(mean=round(mean(estimate),2),upper=round(quantile(estimate,0.975),2),lower=round(quantile(estimate,0.025),2)) 

parmeltP<-parmelts[parmelts$parameter %in% c("linear","polynomial"),]

#calculate CI interval
finaltabP<-group_by(parmeltP,L1,Assemblage,parameter) %>% summarize(mean=round(mean(estimate),2),upper=round(quantile(estimate,0.975),2),lower=round(quantile(estimate,0.025),2)) 

#plot simulated
simulated<-ggplot(parmeltP[parmeltP$Assemblage %in% "Simulation",],aes(x=estimate,fill=parameter)) + facet_wrap(~L1,scale="free_y") + geom_histogram() + theme_bw() + ggtitle("Simulated") + scale_fill_discrete(guide="none") + labs(x="") + scale_x_continuous(limits=c(-45,25)) + scale_fill_manual(values=c("grey60","grey10"))
...
predicted<-ggplot(parmeltP[parmeltP$Assemblage %in% "Predicted",],aes(x=estimate,fill=parameter)) + facet_wrap(~L1,scale="free_y") + geom_histogram() + theme_bw() + ggtitle("Predicted")+ scale_fill_discrete(guide="none") + labs(x="")+ scale_x_continuous(limits=c(-45,25))+ scale_fill_manual(values=c("grey60","grey10"))
...
observed<-ggplot(parmeltP[parmeltP$Assemblage %in% "Observed",],aes(x=estimate,fill=parameter)) + facet_wrap(~L1,scale="free_y") + geom_histogram() + theme_bw() + ggtitle("Observed")+ scale_x_continuous(limits=c(-45,25))+ scale_fill_manual(values=c("grey60","grey10"))
...
```{r view slope species,fig.height=6}
...
trajsplit<-lapply(trajsplit,function(x){
  x[sample(1:nrow(x),1000,replace=F),]
})
...
for(x in 1:length(trajsplit)){
  a<-trajsplit[[x]]
  dat<-sdat[[which(names(sdat) %in% names(trajsplit)[x])]]$Phylo.Relatedness
  
  est<-list()
  for(p in 1:nrow(a)){
    est[[p]]<-data.frame(Relatedness=dat,y=trajF(a[p,]$intercept,a[p,]$linear,a[p,]$polynomial,dat))
    }
  est<-rbind_all(est)
...
#add in observed predicted and eventually simulated labels
specdf[specdf$Assemblage %in% c("Lottery","Niche Cons","Niche Cons + comp"),"Type"]<-"Simulation"

specdf[specdf$Assemblage %in% c("Env","Env + Dispersal","Observed"),"Type"]<-"Observed and Predicted"


#overlay together
ggplot(data=specdf[specdf$Assemblage %in% c("Env","Env + Dispersal","Observed"),],aes(x=Relatedness,y=mean,ymin=lower,ymax=upper,fill=Assemblage)) + geom_ribbon(alpha=.5) + geom_line() + theme_bw()+ labs(x="Distance to closest related species",y="Probability of Occurrence") + scale_fill_manual(values=c("grey80","grey60","black"))

ggsave("Figure3Overlay.jpeg",height=6,width=8,dpi=400)
```

#Simulated trajectories
```{r,fig.width=11}
#overlay together
ggplot(specdf[!specdf$Assemblage %in% c("Env","Env + Dispersal","Observed"),],aes(x=Relatedness,y=mean,ymin=lower,ymax=upper,fill=Assemblage)) + geom_ribbon(alpha=.5) + geom_line() + theme_bw()+ labs(x="Distance to closest related species",y="Probability of Occurrence") + scale_fill_manual(values=c("grey80","grey60","black")) + facet_wrap(~Assemblage)

ggsave("Figure4Overlay.jpeg",height=6,width=10,dpi=400)
