...
#create folder name
  ...
  test<-Bayes(presence = sdat[[x]]$P_A,distance = sdat[[x]]$Phylo.Relatedness,species=sdat[[x]]$Species,runs = 30000,burn=28000,folder = fold)
}
stopCluster(cl)
...
gdf<-parmelts[parmelts$L1 %in% c("Env","Env + Dispersal","Observed"),]
...
ggplot(specdf,aes(x=Relatedness,y=mean)) + geom_line(col="red",linetype="dashed") + geom_line(aes(y=upper),col="blue",linetype="dashed") + geom_line(aes(y=lower),col="blue",linetype="dashed")+ theme_bw() + labs(x="",y="") + facet_wrap(~Assemblage)

#overlay together
ggplot(specdf,aes(x=Relatedness,y=mean,ymin=lower,ymax=upper,fill=Assemblage)) + geom_ribbon(alpha=.5) + geom_line() + theme_bw()+ labs(x="Distance to closest related species",y="Probability of Occurrence") + scale_fill_manual(values=c("grey80","grey60","black"))

ggsave("FigureOverlay.jpeg",height=6,width=8,dpi=400)
