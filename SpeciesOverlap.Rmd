...
costThresh<-apply(siteXspp.raster,1,function(y){
  ...
  outH<-as.matrix(CostPathMatrix[sites,sites])
...
Blue Triangles = predicted assemblage lists
...
for(x in 1:nlayers(modlist)){
  ...
  ppEnv<-PA_m2[PA_m2$Species==gsub(speciesname,pattern=" ",replacement="\\.") & PA_m2$P_R=="P_Apred" & PA_m2$P_A==1 & PA_m2$Hyp=="Env","Locality"]
  
  ptsEnv<-xyFromCell(object=nm, cell=ppEnv$Locality,spatial=T)
  
  #predicted presence points, which are not in the dispersal
  ppDisp<-PA_m2[PA_m2$Species==gsub(speciesname,pattern=" ",replacement="\\.") & PA_m2$P_R=="P_Apred" & PA_m2$P_A==1 & PA_m2$Hyp=="Env_Dispersal","Locality"]
  noDD<-ppDisp[!ppDisp$Locality %in% ppEnv$Locality]
  
  
  #plot niche model and results
  op <- par(mar=rep(.01, 4))
  plot(nm,axes=F,legend=T,box=F,ext=extent(ptsEnv)*1.1)
  par(op)
  points(ptsTrue,col='red',pch=16)

  points(ptsEnv,col='blue',pch=2,cex=1)
  points(sp.loc,cex=.6,pch=16)
  title(paste(speciesname,paste("n =",length(sp.loc)),sep=" "))
  
  #If all env points are in dispersal, ignore
  #If there no points, skip

  if(!length(noDD$Locality)==0){
      ptsDisp<-xyFromCell(object=nm, cell=noDD$Locality,spatial=T)
