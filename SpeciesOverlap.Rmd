...
#plot a few rasters
plot(stack(modlist),maxnl=100,nr=length(modlist)/5)
```
...
runsl<-rep(10000,50000,10000,10000,10000,10000,10000)
...
figure4 + geom_point(data=sdat[[2]],aes(x=Phylo.Relatedness,y=P_A),alpha=0.01)
...
gdf<-group_by(specSdf,Species) %>% filter(str_detect(parameter,"beta")) %>%summarize(Lower=quantile(estimate,0.025),Upper=quantile(estimate,0.975),mean=mean(estimate))

ord<-gdf%>% arrange(desc(mean))

gdf$Species<-factor(gdf$Species,levels=ord$Species)

#merge with clades
gdf<-merge(gdf,clades[,!colnames(clades) %in% "Species"],by.x="Species",by.y="double.")

#split into two panels?
gdf<-gdf[order(gdf$mean,decreasing = T),]
gdf$panel<-cut(1:nrow(gdf),2,c(1,2))

ggplot(gdf,aes(x=Species,col=Clade)) + geom_linerange(aes(ymin=Lower,ymax=Upper)) + geom_point(aes(y=mean)) + theme_bw() + theme(axis.text.x = element_text(angle = 65, hjust = 1)) + ylab("Linear Coefficient") + facet_wrap(~panel,scales="free_x",ncol=1)
ggsave("PolynomialEstimates.svg",dpi=300,height=7,width=8)
ggsave("PolynomialEstimates.jpeg",dpi=300,height=7,width=8)
```
...
##Approximate Bayesian figure
```{r}
#parameter category
parmelt$param<-as.factor(str_match(parmelt$parameter,"\\w+"))
parmelt$ind<-as.numeric(str_extract(parmelt$parameter,pattern="\\d+"))

#species index
#get mean values for all positions
tdf<-group_by(parmelt,L1,param,ind) %>% filter(Assemblage %in% c("Observed","Simulation")) %>%  filter(str_detect(param,c("beta","alpha","gamma"))) %>% summarize(mean=mean(estimate))

#cast to view all parameters
abc<-dcast(tdf,... ~ param)

#label rows
rownames(abc)<-paste(abc$L1,abc$ind)
prinabc<-prcomp(abc[,-c(1:2)],scale=T)

#information about the pca matrix
assemblage<-str_extract(rownames(abc),"\\D+")
pcaspecies<-str_extract(rownames(abc),"\\d+")

#assemblage data
pcadat<-data.frame(Assemblage=assemblage,Species=as.character(pcaspecies),x=prinabc$x[,1],y=prinabc$x[,2],Scenerio=abc$L1)
pcadat$Species<-as.character(pcadat$Species)

#get a unique row number if we need to sort later
pcadat$ID<-1:nrow(pcadat)

#get species index
pcadat[pcadat$Assemblage %in% "Observed ","Species"]<-unique(sdat[[2]]$Species)

#turn off labels for simulated species
pcadat[!pcadat$Assemblage %in% "Observed ","Species"]<-NA

clades$lab<-paste(str_extract(clades$Genus,"\\w"),".",sep="",clades$Species)

#merge clades
pcadat<-merge(pcadat,clades,by.x="Species",by.y="double.",all.x=T,sort=F)

#just get first intial for
#merge patagona, mt gems and topazes
levels(pcadat$Clade)[levels(pcadat$Clade) %in% c("Patagona","Topazes","MtGem","Bee")]<-"Patagona/Topazes/MtGem/Bee"

#ensure we have the proper order
pcadat<-pcadat[order(pcadat$ID),]

#order the labels
abc$L1<-factor(abc$L1,levels=unique(abc$L1)[c(3,1,2,4,5)])

#i had to make up a new function
df<-prinabc$x
d <- prinabc$sdev
v <- prinabc$rotation
  
mybiplot(df,d,v,scale=0,groups =abc$L1,fac=pcadat$Clade,labels=pcadat$lab,var.axes=F) + labs(col="Scenerio")

ggsave("Speciesbiplot.jpeg",dpi=300,height=15,width=15)
ggsave("Speciesbiplot.svg",dpi=300,height=15,width=15)
