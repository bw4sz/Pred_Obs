...
if(!hier){
  source(paste(gitpath,"Bayesian/Single.R",sep="/"))
    #Intials
  InitStage <- function() {list(intercept=0,linear=0,polynomial=0)}
...
runsl<-c(100000,100000,400000,100000,100000,100000)

#Which model we want to draw , heir=T is for full model
modhier<-c(F,T,T,T,F,F)

#cl<-snow::makeCluster(8,"SOCK")
#doSNOW::registerDoSNOW(cl)
parall<-list()
#parall<-foreach(x=1:length(sdat),.errorhandling="stop") %dopar% {
for(x in 1:length(sdat)){
  out<-Bayes(presence=sdat[[x]]$P_A,distance=sdat[[x]]$Phylo.Relatedness,species=sdat[[x]]$Species,runs = runsl[x],burn=(runsl[x]-3000)/2,hier=modhier[x])
  #return(out)
  parall[[x]]<-out
}
#stopCluster(cl)

save.image(paste(droppath,"Thesis\\Pred_Realized\\Assemblages\\Threshold0.05\\Results\\","PredictedRealized.RData",sep=""))
...
#plot simulated
simulated<-ggplot(parmeltP[parmeltP$Assemblage %in% "Simulation",],aes(x=estimate,fill=parameter)) + facet_wrap(~L1,scale="free_y") + geom_histogram() + theme_bw() + ggtitle("Simulated") + scale_fill_discrete(guide="none") + labs(x="") + scale_x_continuous(limits=c(-10,10)) + scale_fill_manual(values=c("grey60","grey10"))
...
predicted<-ggplot(parmeltP[parmeltP$Assemblage %in% "Predicted",],aes(x=estimate,fill=parameter)) + facet_wrap(~L1,scale="free_y") + geom_histogram() + theme_bw() + ggtitle("Predicted")+ scale_fill_discrete(guide="none") + labs(x="")+ scale_x_continuous(limits=c(-30,30))+ scale_fill_manual(values=c("grey60","grey10"))
