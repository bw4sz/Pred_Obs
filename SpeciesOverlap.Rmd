...
PA_m2<-predAssemblages(thresh=.1,plots=TRUE)   
```
...
Bayes<-function(presence,distance,species,runs,burn,folder){
...
parall<-foreach(x=1:length(sdat),.errorhandling="stop",.export=c("gitpath")) %dopar% {
#create folder name
  fold<-paste(unique(sdat[[x]]$P_R),unique(sdat[[x]]$Hyp),sep="_")
  
  test<-Bayes(presence = sdat[[x]]$P_A,distance = sdat[[x]]$Phylo.Relatedness,species=sdat[[x]]$Species,runs = 20000,burn=18000,folder = fold)
}
stopCluster(cl)
...
```{r,fig.height=10}
ggplot(parmelt,aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + facet_grid(L1~parameter,scales="free") + theme_bw() + labs(col="Chain")
```
...
finaltab<-group_by(parmelt,L1,Assemblage,parameter) %>% summarize(mean=round(mean(estimate),2),upper=round(quantile(estimate,0.975),2),lower=round(quantile(estimate,0.025),2)) 

#plot simulated
simulated<-ggplot(parmelt[parmelt$Assemblage %in% "Simulation",],aes(x=estimate,fill=parameter)) + facet_wrap(~L1,scale="free_y") + geom_histogram() + theme_bw() + ggtitle("Simulated") + scale_fill_discrete(guide="none") + labs(x="") + scale_x_continuous(limits=c(-20,20)) + scale_fill_manual(values=c("grey60","grey10"))

#CI intervals
simulated<-simulated + geom_vline(data=finaltab[finaltab$Assemblage %in% "Simulation",],aes(xintercept=mean,group=parameter),linetype="dashed",col="black",size=1) + geom_vline(data=finaltab[finaltab$Assemblage %in% "Simulation",],aes(xintercept=upper,group=parameter),linetype="dashed",col="grey20",size=.5) + geom_vline(data=finaltab[finaltab$Assemblage %in% "Simulation",],aes(xintercept=lower,group=parameter),linetype="dashed",col="grey20",size=.5)

predicted<-ggplot(parmelt[parmelt$Assemblage %in% "Predicted",],aes(x=estimate,fill=parameter)) + facet_wrap(~L1,scale="free_y") + geom_histogram() + theme_bw() + ggtitle("Predicted")+ scale_fill_discrete(guide="none") + labs(x="")+ scale_x_continuous(limits=c(-20,20))+ scale_fill_manual(values=c("grey60","grey10"))

#CI intervals
predicted<-predicted + geom_vline(data=finaltab[finaltab$Assemblage %in% "Predicted",],aes(xintercept=mean,group=parameter),linetype="dashed",col="black",size=1) + geom_vline(data=finaltab[finaltab$Assemblage %in% "Predicted",],aes(xintercept=upper,group=parameter),linetype="dashed",col="grey20",size=.5) + geom_vline(data=finaltab[finaltab$Assemblage %in% "Predicted",],aes(xintercept=lower,group=parameter),linetype="dashed",col="grey20",size=.5)

observed<-ggplot(parmelt[parmelt$Assemblage %in% "Observed",],aes(x=estimate,fill=parameter)) + facet_wrap(~L1,scale="free_y") + geom_histogram() + theme_bw() + ggtitle("Observed")+ scale_x_continuous(limits=c(-20,20))+ scale_fill_manual(values=c("grey60","grey10"))

#CI intervals
observed<-observed + geom_vline(data=finaltab[finaltab$Assemblage %in% "Observed",],aes(xintercept=mean,group=parameter),linetype="dashed",col="black",size=1) + geom_vline(data=finaltab[finaltab$Assemblage %in% "Observed",],aes(xintercept=lower,group=parameter),linetype="dashed",col="grey20",size=.5) + geom_vline(data=finaltab[finaltab$Assemblage %in% "Observed",],aes(xintercept=upper,group=parameter),linetype="dashed",col="grey20",size=.5)
...
```{r,size=8,fig.width=13}

kable(as.data.frame(finaltab))

write.csv(finaltab,"ParameterEstimates.csv")

ggplot(finaltab,aes(x=L1,col=Assemblage,shape=parameter),size=3) + geom_pointrange(aes(y=mean,ymax=upper,ymin=lower),size=1.5) + theme_bw() + facet_wrap(~parameter)
